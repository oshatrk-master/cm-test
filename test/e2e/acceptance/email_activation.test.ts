...
import AuthentificationService = require('../../../api/services/AuthentificationService.js');
...
describe('#2 Подтверждение email', function () {
  ...
  describe('Вход в систему', function () {
    ...
    it('разрешен после подтверждения email', function () {
      ...
      // Подтверждаем e-mail:
      let {id, sah} = pgdb.querySync('SELECT id, password as sah FROM users WHERE username=$1::text', [login])[0];
      const activationLink = 'http://localhost:1337/confirm/' +
        encodeURIComponent(
          AuthentificationService.createEmailConfirmationTokenSync({id, saltAndHash: sah})
        );
